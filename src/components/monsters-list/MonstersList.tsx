...
const MonstersList: React.FC<MonstersListProps> = ({ monsters }) => {
  ...
  const handleMonsterClick = (monster: Monster) => {
    ...
    //selecting a computer monster randomly.
    const filteredMonster = monsters.filter(
      (elMonster) => elMonster.id !== monster.id,
    );
    const randomIndex = Math.floor(Math.random() * filteredMonster.length);
    const computerMonster = filteredMonster[randomIndex];
